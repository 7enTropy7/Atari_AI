...
def train_agent():
    ...
    while True:
        ...
        action_prob = agent.predict_on_batch(state.reshape(1, 30, 30, 1))[0,:]
        action = np.random.choice(18, p=action_prob)
        ...
        if done:
            ...
            X = np.vstack(states).reshape(-1, 30, 30, 1)
            Y = action_probs + lr * rewards * action_prob_grads
            
            agent.train_on_batch(X, Y)

            agent.save_weights('Boxing_agent')
